...
import config_srcnn_v3
from config_srcnn_v3 import Config
...
if __name__ == "__main__":
    ...
    if configs.loss == 'L1':
        ...
        crierion = nn.MSELoss(reduction='mean')
    ...
    criterion_perc = perceptual_loss.PerceptualLoss(configs.num_features, configs.kernel_size_1, configs.stride_1,
                                                    configs.kernel_size_2, configs.stride_2, configs.kernel_size_3,
                                                    resize=None)
    criterion_novel = MS_SSIM(data_range=1.0, win_size=1, size_average=True, spatial_dims=3, channel=1)
    if configs.model == 'UNET':
        model = modified_unet_1.UNet(configs.num_features, configs.kernel_size_1, configs.stride_1, configs.kernel_size_2, configs.stride_2, configs.kernel_size_3)
        model.to(device)
        ...
        try:
            if os.path.exists(fname):
                ...
                model, optimizer, start_epoch, valid_loss_min, scaler = checkpoints.load_ckp(configs.checkpointpath, model,
                                                                                          optimizer, scaler)
                print(start_epoch)
                _, valid, train = train_ensemble.train(configs.epochs - start_epoch, training_loader, validation_loader, valid_loss_min, optimizer,
                            model,
                ...
                _, valid, train = train_ensemble.train(configs.epochs, training_loader, validation_loader, valid_loss_min, optimizer, model,
                            writer_train,
        ...
        model, optimizer, start_epoch, valid_loss_min, scaler = checkpoints.load_ckp(configs.bestmodelckp, model,
                                                                             optimizer, scaler)


        #tensor_vis = tensor_vis[32:33, :, :]
    elif configs.model == 'SRDENSENET':
        activation_maps = configs.scale_factor ** configs.num_dimensions
        model = SRDenseNet.SRDenseNet()
        model.to(device)
        criterion = nn.L1Loss(reduction='mean')
        optimizer = optim.Adam(model.parameters(), lr=configs.learningrate)
        scaler = GradScaler()
        training_set_down, validation_set_down, training_set_original, validation_set_original = srcnn_subject_loading.subject_loading(
        ...
        training_loader_down, validation_loader_down = patch_loading.patch_Loading_unet(patch_size,
                                                                                                          samples_per_volume,
                                                                                                          max_queue_length,
                                                                                                          training_set_down,
                                                                                                          validation_set_down,
                                                                                                          ...
                                                                                                          configs.training_batch_size,
                                                                                                          configs.validation_batch_size)
        training_loader_original, validation_loader_original = patch_loading.patch_Loading_unet(
            patch_size_orig, samples_per_volume,
            max_queue_length, training_set_original,
            validation_set_original,
            configs.patch_check,
            configs.training_batch_size,
            configs.validation_batch_size)
        ...
        try :
            if os.path.exists(fname) :
                ...
                model, optimizer, start_epoch, valid_loss_min, scaler = checkpoints.load_ckp(configs.checkpointpath, model,
                                                                                     optimizer, scaler)
                print(start_epoch)
                _, valid, train = train_ensemble.train(configs.epochs - start_epoch, training_loader_down,
                                              validation_loader_down, valid_loss_min,
                                              optimizer, model,
                                              writer_train, writer_valid, writer_images, criterion, criterion_perc, criterion_novel,
                                              configs.checkpointpath, configs.bestmodelckp, configs.model, configs.mode, configs.loss, scaler)
            else :
                _, valid, train = train_ensemble.train(configs.epochs, training_loader, validation_loader, valid_loss_min,
                                              optimizer, model,
                                              writer_train,
                                              writer_valid, writer_images,  criterion, criterion_perc, criterion_novel, configs.checkpointpath,
                                              configs.bestmodelckp, configs.model, configs.mode, configs.loss, scaler)
        except MemoryError as error :
            print('memory error')
