...
class SRCNN3Dv2(nn.Module) :
    def __init__(self, n_channels=1, scale_factor=2, num_features=32, kernel_size=3, stride=1) :
        ...
        self.conv_2 = nn.Sequential(
            nn.Conv3d(num_features, num_features, kernel_size, stride, padding=kernel_size // 2),
            nn.BatchNorm3d(num_features=num_features), nn.ReLU(inplace=True))
        ...
        for i in range(len(self.scale_factor)) :
            if self.scale_factor[i] > 1 :
                ...
                self.conv_6s.append(
                    nn.Sequential(nn.Conv3d(num_features, out_features, kernel_size, stride, padding=kernel_size // 2),
                                  nn.BatchNorm3d(num_features=out_features), nn.ReLU(inplace=True)))
                self.conv_6s_post.append(nn.Sequential(
                    nn.Conv3d(out_features // self.scale_factor[i], num_features, kernel_size, stride,
