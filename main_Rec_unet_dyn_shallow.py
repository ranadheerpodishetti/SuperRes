...
from config_rec_unet_dyn_shallow import Config
import dynamic_unet
import Rec_UNet_with_dynconv_shallow
...
if __name__ == "__main__":
    ...
    if configs.loss == 'L1':
        ...
        criterion = nn.L1Loss(reduction='none')
    criterion_perc = nn.L1Loss(reduction='none')
    ...
    if configs.model == 'UNET':
        ...
        model = Rec_UNet_with_dynconv_shallow.R2AttU_Net(configs.num_features, 1, configs.kernel_size_1, configs.stride_1,
                                                         configs.kernel_size_2, configs.stride_2, configs.kernel_size_3)
        model.to(device)
