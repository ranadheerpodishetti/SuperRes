                ...
                if Model == 'UNET':
                    ...
                    if mode != 'DS':
                        ...
                        if loss_func == 'L1':
                            ...
                            loss = criterion(output, labels)
                        elif loss_func == 'NOVEL':
                            loss_l1 = criterion(output, labels)
                            print(type(labels.data), type(output.data))
                            print(labels.dtype, output.dtype)
                            if labels.dtype != output.dtype:
                                loss_perc = criterion_perc(output.type(torch.cuda.FloatTensor), labels)
                            else:
                            ...
                            loss = (loss_l1 * 0.4 + loss_perc * 0.6) / 2.0
                        else:
                            loss_l1 = criterion(output, labels)
                            loss_perc = criterion_perc(output, labels)
                            ...
                            gaussian = torch.normal(1, 0.5, loss_l1.shape).to(device)
                            loss_l1_out = torch.mean(torch.mul(loss_l1, gaussian)).to(device)
                            print(loss_l1_out, loss_msssim, loss_perc)
                            loss = (loss_l1_out*0.4 + loss_msssim*0.6 + loss_perc*0.2)/3.0
                            print(type(loss))
                    else:
                        print('DS')
                        ...
                        if loss_func == 'L1':
                            ...
                            loss_l1 = criterion(output, labels)
                            print(type(labels, output))
                            loss_msssim = 1.0 - criterion(output.type(torch.cuda.FloatTensor), labels)
                            gaussian = torch.normal(1, 0.5, loss_l1.shape).to(device)
                            loss_l1 = torch.mean(torch.mul(loss_l1, gaussian)).to(device)
                            loss_out = (loss_l1 * 0.16 + loss_msssim * 0.84) / 3.0
                            loss_x3 = criterion(x_3, labels)
                            loss_x4 = criterion(x_4, labels)
                        else:
                            print(1)
                ...
                if Model == 'UNET':
                    ...
                    if mode != 'DS':
                        ...
                        if loss_func == 'L1':
                            ...
                            loss_l1 = criterion(output, labels_val)
                            if labels.dtype != output.dtype:
                                loss_perc = criterion_perc(output.type(torch.cuda.FloatTensor), labels)
                            else:
                                loss_perc = criterion_perc(output, labels)
                        ...
                        if loss_func == 'L1':
                            ...
                            loss_l1 = criterion(output, labels_val)
                            loss_msssim = 1.0 - criterion(output.type(torch.cuda.FloatTensor), labels_val)
                            gaussian = torch.normal(1, 0.5, loss_l1.shape).to(device)
                            loss_l1 = torch.mean(torch.mul(loss_l1, gaussian)).to(device)
                            loss_out = (loss_l1 * 0.16 + loss_msssim * 0.84) / 3.0
                            loss_x3 = criterion(x_3, labels_val)
