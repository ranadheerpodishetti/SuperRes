...
class UNet(nn.Module):
    def __init__(self, num_features, kernel_size_1, stride_1, kernel_size_2, stride_2, kernel_size_3):
        ...
        # self.conv_shuffle = nn.Conv3d(inp, out * (upscale ** 2), 3, padding=1)
        self.pixel_1 = pixel_shuffle_new.pixel_shuffle_new(num_features * 16, num_features * 16 * (2 ** 2), kernel_size_1, stride_1)
    ...
    def forward(self, image):
        ...
        x_3 = F.interpolate(self.conv2(x), (32, 32, 32), mode='trilinear')
        print('x_3', x_3.size())
        print(x.size())
        x = self.pixel_3(x)
        # x = nn.functional.interpolate(x, scale_factor= 2, mode= 'trilinear')
        x = self.up_conv_3(torch.cat([x, x3], 1))
        x_4 = F.interpolate(self.conv3(x), (32, 32, 32), mode='trilinear')
        print('x_4', x_4.size())
        print(x.size())
        x = self.pixel_4(x)
        # x = nn.functional.interpolate(x, scale_factor= 2, mode= 'trilinear')
