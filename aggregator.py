...
orig = 'E:/master/sem2/superres/execution/dummy out/orig.nii.gz'
down_nii = 'E:/master/sem2/superres/execution/dummy out/down.nii.gz'
...
test_set = torchio.ImagesDataset(subjects)
valid_loss_min = np.inf

def grid_aggregator(path, test_set, patch_size, overlap, model, optimizer, scaler, num_workers = 0):
    ...
    # labels = logits.argmax(dim=CHANNELS_DIMENSION, keepdim=True)
        ...
        nib.save(nii_super_res, configs.path)
        nib.save(nii_ground_truth, configs.gt)
        nib.save(nii_input, configs.inp)
...
grid_aggregator(configs.bestmodelckp, test_set, configs.patchsize, configs.patch_overlap, model, optimizer, scaler)