...
import config_dynconv_ssim
from config_dynconv_ssim import Config
import dynamic_unet
...
if __name__ == "__main__":
    ...
    criterion_perc = perceptual_loss.PerceptualLoss(configs.num_features, configs.kernel_size_1, configs.stride_1,
                                                    configs.kernel_size_2, configs.stride_2, configs.kernel_size_3,
                                                    resize=None)
    criterion_novel = SSIM(data_range=1, size_average=True, spatial_dims=3, channel=1)
    if configs.model == 'UNET':
        model = dynamic_unet.UNet(configs.num_features, configs.kernel_size_1, configs.stride_1, configs.kernel_size_2,
                                  configs.stride_2, configs.kernel_size_3)
        model.to(device)
        ...
        try:
            if os.path.exists(fname):
                print('file found')
                model, optimizer, start_epoch, valid_loss_min, scaler = checkpoints.load_ckp(configs.checkpointpath,
                                                                                             model,
                                                                                             optimizer, scaler)
                print(start_epoch)
                _, valid, train = train.train(configs.epochs - start_epoch, training_loader, validation_loader,
                                              valid_loss_min, optimizer,
                                              model,
                                              writer_train, writer_valid, writer_images, criterion, criterion_perc,
                                              criterion_novel, configs.checkpointpath, configs.bestmodelckp,
                                              configs.model, configs.mode, configs.loss, scaler)
            else:
                _, valid, train = train.train(configs.epochs, training_loader, validation_loader, valid_loss_min,
                                              optimizer, model,
                                              writer_train,
                                              writer_valid, writer_images, criterion, criterion_perc, criterion_novel,
                                              configs.checkpointpath, configs.bestmodelckp, configs.model, configs.mode,
                                              configs.loss, scaler)
        ...
        model = SRCNN.SRCNN_late_upscaling(configs.num_features, configs.kernel_size_1, configs.stride_1,
                                           activation_maps)
        model.to(device)
        criterion = nn.L1Loss(reduction='mean')
        optimizer = optim.Adam(model.parameters(), lr=configs.learningrate)
        ...
        try:
            if os.path.exists(fname):
                ...
                model, optimizer, start_epoch, valid_loss_min = checkpoints.load_ckp(configs.checkpointpath, model,
                                                                                     optimizer)
                print(start_epoch)
                _, valid, train = train.train(configs.epochs - start_epoch, training_loader_down,
                                              validation_loader_down, valid_loss_min,
                                              optimizer, model,
                                              writer_train, writer_valid, writer_images, criterion,
                                              configs.checkpointpath, configs.bestmodelckp, configs.model, down_valid,
                                              orig_visualization, down_visualization)
            else:
                _, valid, train = train.train(configs.epochs, training_loader, validation_loader, valid_loss_min,
                                              optimizer, model,
                                              writer_train,
                                              writer_valid, writer_images, criterion, configs.checkpointpath,
                                              configs.bestmodelckp, configs.model, down_valid, orig_visualization,
                                              down_visualization)
        except MemoryError as error:
            print('memory error')
    ...
    model.eval()
    with torch.no_grad():
        output, inputs, labels, loss, loss_list = test.test(test_loader, configs.model, model, criterion,
                                                            configs.mode)  # Ask Raghava
        N = 1
