...
class UNet(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, depth=5, wf=6, padding=False,
        ...
        self.veryLast = nn.Tanh() ## nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        return self.veryLast(self.last(x))
...
class UNet3D(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, depth=5, wf=6, padding=False,
        ...
        self.down_path = nn.ModuleList()
        for i in range(depth):
            self.down_path.append(UNet3DConvBlock(prev_channels, 2 ** (wf + i),
                                                padding, batch_norm))
            prev_channels = 2 ** (wf + i)

        self.up_path = nn.ModuleList()
        for i in reversed(range(depth - 1)):
            self.up_path.append(UNet3DUpBlock(prev_channels, 2 ** (wf + i), up_mode,
                                            padding, batch_norm))
            prev_channels = 2 ** (wf + i)

        self.last = nn.Conv3d(prev_channels, n_classes, kernel_size=1)
        self.veryLast = nn.Sigmoid() # nn.ReLU() # 
...
class UNet3DConvBlock(nn.Module):
    def __init__(self, in_size, out_size, padding, batch_norm):
...
class UNet3DUpBlock(nn.Module):
    def __init__(self, in_size, out_size, up_mode, padding, batch_norm):
        super(UNet3DUpBlock, self).__init__()
        if up_mode == 'upconv':
        ...
        self.conv_block = UNet3DConvBlock(in_size, out_size, padding, batch_norm)

    # TODO: check if its the correct depth index!!!
    def center_crop(self, layer, target_size):
