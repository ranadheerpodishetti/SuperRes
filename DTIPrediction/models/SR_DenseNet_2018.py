...
class _Dense_Block(nn.Module):
    def __init__(self, channel_in):
        ...
        self.relu = nn.ELU() #nn.PReLU() # nn.Tanh()# nn.ReLU() # nn.PReLU()
...
class Net(nn.Module):
    ## in_channels = 7 for self.lowlevel 
    ## out_channels = 7 for self.reconstruction
    def __init__(self):
        super(Net, self).__init__()
        
        self.relu = nn.ELU() ## nn.PReLU()
        
        self.lowlevel = nn.Conv2d(in_channels=7, out_channels=128, kernel_size=3, stride=1, padding=1)
        self.bottleneck = nn.Conv2d(in_channels=1152, out_channels=256, kernel_size=1, stride=1, padding=0, bias=False)
        # self.reconstruction = nn.Conv2d(in_channels=256, out_channels=7, kernel_size=3, stride=1, padding=1, bias=False)
        self.reconstruction = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=7, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Tanh()
            )
    ...
    def forward(self, x):    
        ...
        # ------------------------------------
        # before bottleneck   torch.Size([32, 1152, 128, 128])
        # before deconv   torch.Size([32, 256, 128, 128])
        # before reconstruction   torch.Size([32, 256, 512, 512])
        # after reconstruction torch.Size([32, 7, 512, 512])
        # torch.nn.functional.interpolate(input, size=None, scale_factor=None, mode='nearest',
        # align_corners=None, recompute_scale_factor=None)
        # out  = nn.functional.interpolate(out, scale_factor=(0.25,0.25), mode='nearest')
        return out
