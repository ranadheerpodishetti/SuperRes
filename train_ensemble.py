        ...
        for i, data in enumerate(training_loader, 0):
            ...
            with autocast():
                if Model == 'UNET':
                    ...
                    output  = model(inputs)
                    lossl1_dense = criterion(output, labels)
                    loss_msssim_dense = criterion_novel(output, labels)
...
#model is final ENSEMBLE model
            ...
            loss_nan = np.array(running_loss)
            if np.isnan(loss_nan):
                sys.exit()
        ...
        with torch.no_grad():
            ...
            for l, data in enumerate(validation_loader, 0):
                ...
                if Model == 'UNET':
                    ...
                    lossl1_dense = criterion(output, labels_val)
                    loss_msssim_dense = criterion_novel(output, labels_val)
                    loss = lossl1_dense + loss_msssim_dense
                else:
                    input_val = data[0]['down'][DATA].to(device)
                    labels_val = data[1]['original'][DATA].to(device)
            ...
            if Model == 'UNET':
                ...
                img_grid = make_grid([ground_tensor[4:5, :, :], tensor_vis])
                writer_images.add_image('images', img_grid, global_step = epoch)
